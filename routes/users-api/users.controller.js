...
exports.findAllUsers = function(req, res) {   //exports. sirve para que la funcion sea visible por la instancia de nuestro controlador
...
exports.findByMatricula = function(req, res){
	User.find( {"email" : req.params.email}, function(err, user){ //En este caso ya no es un find directo, sino uno que pide explicitamente 'matricula': variable_matricula_recibida_en_la_URL
		if(err)
			res.send(500, err.message);
...
exports.checkUser = function(req, res){
	User.find( {"email" : req.params.email, "password" : req.params.password}, function(err, user){ //En este caso ya no es un find directo, sino uno que pide explicitamente 'matricula': variable_matricula_recibida_en_la_URL, 'password' : variable_password_recibida_en_la_URL
		if(err)
			res.send(500, err.message);
